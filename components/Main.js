...
const Main = ({ nearId, query }) => {
	...
	const [verifiedArray, setVerifiedArray] = useState(verifiedContracts)
	...
	const render = async () => {
		setVerifiedArray(verifiedContracts);
		const nftContracts = await getFetcher(`https://api.kitwallet.app/account/${nearId}/likelyNFTs`);
